...
@click.group(context_settings={"auto_envvar_prefix": "JOVIAN"})
@click.option("--log_level", show_default=True, type=click.Choice(LOG_LEVELS))
@click.version_option(prog_name="Jovian")
@click.pass_context
def main(ctx, log_level="info"):
    """Try out a demo with:

        $ jovian clone aakashns/jovian-tutoial
    ...
    if log_level:
        print("Setting log level", log_level.upper())
