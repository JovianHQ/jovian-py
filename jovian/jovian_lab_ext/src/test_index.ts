    ...
    async function activate (app: JupyterFrontEnd) {
      ...
      setTimeout(async () => {
        ...
        void session.initialize().then(() => {
          ...
          let a = session.kernel.requestExecute({code: "a = 20\n%store a\nprint(a)\n"});
          //let a = session.kernel.requestExecute({code: "a = 15\n"});
          //console.log("msg:\n" + a.msg.content.code);
          let mmm;
          a.onReply(mmm);
          console.log("msg:\n" + mmm);
        ...
        /*
        const main = (await app.commands.execute(
          'terminal:create-new'
        ));
